...
# Standard library modules - core Python functionality
import sys          # System-specific parameters and functions
import json         # JSON data interchange format handling
import os           # Operating system interface utilities
import csv          # CSV file reading and writing
import io           # Core tools for working with streams
import platform     # Access to underlying platform's identifying data
import numpy as np  # Numerical computing with array operations
from datetime import datetime  # Date and time handling
from typing import List, Dict, Any, Optional, NamedTuple, Tuple  # Type hints for better code clarity

# Optional dependencies for enhanced functionality
try:
    import pandas as pd      # Data analysis and manipulation library
    import openpyxl         # Excel file format support
    EXCEL_AVAILABLE = True  # Excel functionality is available
except ImportError:
    EXCEL_AVAILABLE = False  # Excel features will be disabled

try:
    import cv2              # Computer vision library for image processing
    CV2_AVAILABLE = True    # Advanced image processing available
except ImportError:
    CV2_AVAILABLE = False   # Basic image handling only

try:
    import fitz             # PyMuPDF for PDF document processing
    PDF_AVAILABLE = True    # PDF reading capabilities available
except ImportError:
    PDF_AVAILABLE = False   # PDF reading not available

# ReportLab components for PDF generation
from reportlab.pdfgen import canvas                      # PDF drawing and content creation
from reportlab.lib.pagesizes import letter              # Standard page size definitions
from reportlab.lib.units import inch                    # Unit conversion utilities
from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle  # Text styling
from reportlab.platypus import SimpleDocTemplate, Paragraph, Spacer, Table, TableStyle, KeepTogether  # Advanced layout components
...
from PyQt6.QtGui import QPixmap, QFont, QWheelEvent, QMouseEvent, QPainter, QImage, QColor, QPen, QBrush  # Graphics and mouse/keyboard handling
...
# while transitioning to the centralized AppConfig class structure
MAX_OPTIONS_COUNT = AppConfig.MAX_OPTIONS_COUNT                    # Maximum answer options per question
DEFAULT_POINTS_RANGE = AppConfig.DEFAULT_POINTS_RANGE             # Default point value range
BUBBLE_RADIUS = AppConfig.BUBBLE_RADIUS                           # Bubble detection radius
BUBBLE_SPACING = AppConfig.BUBBLE_SPACING                         # Spacing between bubbles
MIN_BOTTOM_MARGIN = AppConfig.MIN_BOTTOM_MARGIN                   # Minimum page bottom margin
QUESTION_HEIGHT = AppConfig.QUESTION_HEIGHT                       # Height allocation per question
PREVIEW_TEXT_TRUNCATE_LENGTH = AppConfig.PREVIEW_TEXT_TRUNCATE_LENGTH  # Text truncation limit
SPLITTER_SIZES = AppConfig.SPLITTER_SIZES                         # UI panel sizes
PDF_MARGINS = AppConfig.PDF_MARGINS                               # PDF page margins
FONT_SIZES = AppConfig.FONT_SIZES                                 # Typography scale

def get_font():
    """
    Locate and register a suitable Unicode font for PDF generation.
    
    This function searches the system for available TrueType fonts that support
    international characters, particularly important for multi-language support.
    It attempts to register the first available font with ReportLab's font system.
    
    Returns:
        str: Name of the registered font, or 'Helvetica' as fallback
    """
    # Retrieve platform-specific font paths
    font_paths = AppConfig.FONT_PATHS.get(platform.system(), [])
    
    # Attempt to find and register an available font
    for path in font_paths:
        if os.path.exists(path):
            try:
                # Register the font with ReportLab for PDF generation
                pdfmetrics.registerFont(TTFont('UnicodeFont', path))
                return 'UnicodeFont'
            except Exception:
                # Continue searching if font registration fails
                continue
    
    # Fallback to default system font if no custom fonts are available
    return 'Helvetica'

# Initialize global font configuration
FONT = get_font()
...
class FileManager:
    """
    Centralized file operations and dialog management.
    
    This class provides a unified interface for all file-related operations
    throughout the application. It handles file dialogs, format validation,
    and cross-platform compatibility issues to ensure consistent behavior
    ...
    def get_supported_formats():
        """
        Build file filter string for supported formats based on available libraries.
        
        Dynamically constructs a file filter string that includes all supported
        formats based on which optional libraries are available. This ensures
        the file dialogs only show formats that can actually be processed.
        
        Returns:
            str: File filter string formatted for QFileDialog usage
        """
    ...
    def get_safe_filename(file_path: str) -> str:
        """
        Extract filename from path with robust error handling.
        
        Safely extracts just the filename portion from a full file path,
        handling edge cases like empty paths, different path separators,
        and malformed paths across different operating systems.
        
        Args:
            file_path (str): Full path to extract filename from
            
        Returns:
            str: Filename only, or empty string if path is invalid
        """
    ...
    def open_file_dialog(parent, title: str, file_filter: str = None):
        """
        Display cross-platform compatible file selection dialog.
        
        Creates a file selection dialog with appropriate settings for the current
        platform. Includes workarounds for known issues on specific operating
        systems to ensure consistent behavior.
        
        Args:
            parent: Parent widget for modal dialog
            title (str): Dialog window title
            file_filter (str, optional): File type filter string
            
        Returns:
            tuple: (selected_filename, selected_filter)
        """
        if file_filter is None:
            file_filter = FileManager.get_supported_formats()

        # Use non-native dialog on Linux to avoid desktop environment conflicts
    ...
    def save_file_dialog(parent, title: str, file_filter: str, default_suffix: str = ""):
        """
        Display cross-platform compatible file save dialog.
        
        Creates a file save dialog with consistent behavior across platforms,
        including appropriate file extension handling and path validation.
        
        Args:
            parent: Parent widget for modal dialog
            title (str): Dialog window title
            file_filter (str): File type filter string
            default_suffix (str): Default file extension to append
...
class ErrorHandler:
    """
    Dealing with when things go wrong (spoiler: they always do).
    
    Rather than having error dialogs scattered everywhere with inconsistent
    messages, this centralizes all the "oops, something broke" handling.
    """

    @staticmethod
    def show_error(parent, title: str, message: str, details: str = None):
        """
        Show a scary red error dialog.
        
        When something breaks badly enough that the user needs to know,
        this pops up a dialog with a clear explanation and optional
        technical details for the curious.
        
        Args:
            parent: What window owns this dialog
            title (str): Title bar text
            message (str): The main "what went wrong" message
            details (str, optional): Technical details for debugging
            
        Returns:
            int: Which button the user clicked (usually just OK)
        """
        ...
        if details:
            msg_box.setDetailedText(details)  # Hidden by default, expandable
        return msg_box.exec()

    @staticmethod
    def show_warning(parent, title: str, message: str):
        """
        Show a yellow "heads up" warning.
        
        For when something's not quite right but not catastrophic.
    ...
    def show_info(parent, title: str, message: str):
        """
        Show a friendly blue info dialog.
        
        For general information and "hey, that worked!" messages.
    ...
    def handle_file_error(parent, operation: str, file_path: str, error: Exception):
        """
        Show a nice error when file operations go wrong.
        
        File operations fail constantly (permissions, missing files, etc.).
        This shows a user-friendly message plus technical details.
        
        Args:
            parent: Parent window
            operation (str): What we were trying to do ("save", "load", etc.)
            file_path (str): Path to the problematic file
            error (Exception): The actual error that happened
        """
    ...
    def safe_execute(func, error_callback=None, *args, **kwargs):
        """
        Run a function and catch any explosions.
        
        Wraps potentially dangerous operations in try-catch and handles
        errors gracefully. Useful for operations that might fail but
        shouldn't crash the whole app.
        
        Args:
            func: The function to run
            error_callback: What to call if it breaks (optional)
            *args: Arguments for the function
            **kwargs: Keyword arguments for the function
            
        Returns:
            tuple: (result, error) - result is None if something broke
        """
        try:
...
class UIHelpers:
    ...
    def show_message(parent: QWidget, msg_type: str, title: str, message: str):
        """
        Display a styled message dialog with appropriate iconography.
        
        Creates message dialogs with consistent styling and appropriate icons
        based on the message type. Provides visual feedback that matches
        the semantic meaning of the message content.
        
        Args:
            parent (QWidget): Parent widget for dialog positioning
            msg_type (str): Message type ("success", "error", "warning")
            title (str): Dialog window title
            message (str): Message content to display
        """
        # Map semantic message types to appropriate dialog icons
        icons = {
...
class Translator:
    ...
    Handles all the text translation stuff so the app can work in different
    languages. Currently does English and Greek, but you could add more
    languages pretty easily. Has fallbacks so nothing breaks if translations
    are missing.
    """
    ...
    def _initialize_translations(self):
        """
        Load up all the translations for different languages.
        
        This is where all the text translations live. English is the base
        language and everything else falls back to English if a translation
        is missing.
    ...
    def set_language(self, lang_code: str):
        """
        Switch to a different language if we support it.
        
        Changes the active language for the whole app. If you ask for
        a language we don't have, we just ignore you and stick with
        what we've got.
        
        Args:
            lang_code (str): Language code like 'en' or 'el'
        """
    ...
    def t(self, key: str) -> str:
        ...
        This is the main function everyone uses to get translated text.
        If the current language doesn't have the text, fall back to English.
        If English doesn't have it either, return something obvious so
        we know we forgot to translate something.
        
        Args:
            key (str): The key to look up (like 'menu_file')
            
        Returns:
            str: Translated text, or a fallback if not found
        """
...
# Create a global translator that everyone can use
translator = Translator()


def get_option_letter(index: int) -> str:
    """
    Get the right letter for answer options based on language.
    
    In English we use A, B, C, D. In Greek they use Α, Β, Γ, Δ.
    This figures out which one to use based on the current language.
    
    Args:
        index (int): Which option (0 = first, 1 = second, etc.)
...
class SignalBlocker:
    """
    Temporarily stop Qt widgets from sending signals.
    
    Sometimes you need to update a bunch of UI elements without triggering
    their change events. This context manager blocks signals while you
    work, then turns them back on when you're done.
    
    Usage:
        with SignalBlocker(combo1, combo2, slider):
            combo1.setCurrentIndex(5)  # Won't trigger signals
            combo2.setCurrentText("Something")
            slider.setValue(50)
        # Signals are back on now
    """

    def __init__(self, *widgets):
        """
        Set up signal blocking for a bunch of widgets.
        
        Args:
            *widgets: All the widgets you want to silence temporarily
        """
        self.widgets = widgets

    def __enter__(self):
        """Block signals when entering the context."""
        for widget in self.widgets:
            widget.blockSignals(True)
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        """Turn signals back on when leaving the context."""
        for widget in self.widgets:
...
class Question:
    ...
    def __init__(self):
        """Create a new question with sensible defaults."""
        self.text: str = ""  # The actual question text
        self.options: List[str] = ["Option A", "Option B", "Option C", "Option D"]  # Standard 4 choices
        self.correct: int = 0  # First option (A) is correct by default
        self.points: int = 1   # Worth 1 point (because that's normal)
...
class Form:
    ...
    def __init__(self):
        ...
        self.title: str = "New Form"  # Default title
        self.instructions: str = "Select the best answer for each question."  # Default instructions
        self.questions: List[Question] = []  # Empty questions list
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        Returns:
            dict: Dictionary representation of the complete form
        """
        return {
            'title': self.title,
            'instructions': self.instructions,
            'questions': [q.to_dict() for q in self.questions]  # Serialize all questions
        }
...
# ============================================================================
# SCANNER AND GRADING DATA MODELS
# ============================================================================
...
class GradeResult(NamedTuple):
    """
    Complete grading result for a student's OMR sheet.
    
    Contains all information about a student's test performance including
    individual answers, scoring details, and statistical breakdown.
    
    Attributes:
        student_name (str): Student's full name
        student_id (str): Student identification number
        answers (Dict[int, str]): Student's answers by question number
        correct_answers (Dict[int, str]): Correct answers by question number
        points_per_question (Dict[int, int]): Point values by question number
        score (int): Total points earned by student
        total_possible (int): Maximum possible points on test
        percentage (float): Percentage score (0.0-100.0)
        correct_count (int): Number of correct answers
        incorrect_count (int): Number of incorrect answers
        blank_count (int): Number of blank/unanswered questions
        question_results (Dict[int, bool]): Per-question results (True=correct, False=wrong, None=blank)
    """
    student_name: str
    student_id: str
...
class GradingSystem:
    ...
    def __init__(self):
        """Initialize grading system with empty results collection."""
    ...
    def calculate_grade(self, student_name: str, student_id: str,
        ...
        """
        Calculate complete grade for a student's test.
        
        Compares student answers against the answer key, calculates scores,
        and generates comprehensive grading results including statistics.
        
        Args:
            student_name (str): Student's full name
            student_id (str): Student identification number
            student_answers (Dict[int, str]): Student's answers by question number
            answer_key (Dict[int, str]): Correct answers by question number
            points_per_question (Dict[int, int]): Point values by question number
            
        Returns:
            GradeResult: Complete grading result for the student
        """
...
class ZoomableImageLabel(QLabel):
    ...
    def __init__(self):
        ...
        # Bubble manipulation properties
    ...
    def resizeEvent(self, event):
        """Handle widget resize to refit image"""
        super().resizeEvent(event)
        if self.original_image:
            # Refit image when widget is resized
            self.fit_to_window()
    ...
    def paintEvent(self, event):
        """Custom paint with pan support and bubble overlays"""
        ...
        if self.current_pixmap and not self.current_pixmap.isNull():
            ...
            # Then draw bubble overlays if in drag mode
                    ...
                    for q_num, question_bubbles in self.bubble_coordinates.items():
                        ...
                        for option, bubble_data in question_bubbles.items():
                            ...
                            try:
                                ...
                                radius = float(bubble_data.get('radius', 20))
                            ...
                            try:
                                if (self.hover_bubble and len(self.hover_bubble) >= 2 and 
                                    ...
                                    color = QColor(255, 165, 0, 150)  # Orange for hover
                                    ...
                                    color = QColor(255, 0, 0, 150)    # Red for dragging
                                else:
                                    color = QColor(0, 255, 0, 100)    # Green for normal
                            except Exception:
                                color = QColor(0, 255, 0, 100)  # Default to green
                            ...
                            try:
                                ...
                                continue  # Skip this bubble if drawing fails
                                
                except Exception as e:
                    print(f"ERROR in paintEvent bubble overlay: {e}")
                    # Continue without bubble overlays
        else:
    ...
    def mousePressEvent(self, event: QMouseEvent):
        try:
            # Handle drag mode for bubble manipulation
            ...
            # Handle panning (existing functionality)
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        try:
            ...
            if self.dragging_bubble:
                ...
                # Validate dragging_bubble structure
                if not isinstance(self.dragging_bubble, tuple) or len(self.dragging_bubble) != 3:
                    print("ERROR: Invalid dragging_bubble structure")
                    self.dragging_bubble = None
                    return
                ...
                # Validate bubble_data
                if not isinstance(bubble_data, dict) or 'x' not in bubble_data or 'y' not in bubble_data:
                    print("ERROR: Invalid bubble_data structure")
                    self.dragging_bubble = None
                    return
                
                # Ensure zoom_factor is valid to prevent division by zero
                if self.zoom_factor <= 0:
                    self.zoom_factor = 1.0
                
                # Convert delta to image coordinates
                image_delta_x = delta.x() / self.zoom_factor
                image_delta_y = delta.y() / self.zoom_factor
                
                # Update bubble coordinates safely
                try:
                    current_x = float(bubble_data['x'])
                    current_y = float(bubble_data['y'])
                    bubble_data['x'] = current_x + image_delta_x
                    bubble_data['y'] = current_y + image_delta_y
                except (ValueError, TypeError, KeyError) as e:
                    print(f"ERROR updating bubble coordinates: {e}")
                    self.dragging_bubble = None
                    return
            ...
            # Handle hover detection in drag mode
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        try:
            # Handle bubble drag completion
            if self.dragging_bubble:
                # Notify about bubble position change
                if self.bubble_update_callback:
                    try:
                        self.bubble_update_callback(self.bubble_coordinates)
                    except Exception as e:
                        print(f"ERROR in bubble_update_callback: {e}")
                        import traceback
                        traceback.print_exc()
    ...
    def set_drag_mode(self, enabled: bool):
        """Enable or disable bubble drag mode"""
        ...
        try:
            ...
            for q_num, question_bubbles in self.bubble_coordinates.items():
                for option, bubble_data in question_bubbles.items():
                    ...
                    bubble_x = bubble_data.get('x', 0)
                    bubble_y = bubble_data.get('y', 0)
                    radius = bubble_data.get('radius', 20)
                    
                    # Ensure numeric values
                    try:
                        bubble_x = float(bubble_x)
                        bubble_y = float(bubble_y)
                        radius = float(radius)
                    except (ValueError, TypeError):
                        continue
                    
                    # Check if position is within bubble
            ...
            print(f"ERROR in get_bubble_at_position: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def screen_to_image_coords(self, screen_pos: QPoint):
        ...
        try:
            ...
            # Get the actual displayed pixmap rect
            pixmap_rect = self.current_pixmap.rect()
            widget_rect = self.rect()
            
            # Calculate the position of the pixmap within the widget
            ...
            print(f"ERROR in screen_to_image_coords: {e}")
            return screen_pos
    ...
    def image_to_screen_coords(self, image_pos: QPoint):
        ...
        try:
            ...
            print(f"ERROR in image_to_screen_coords: {e}")
            return image_pos
    ...
    def load_file_preview(self, filename: str):
        try:
            ...
            if ext.endswith('.csv'):
                ...
                ErrorHandler.show_warning(self, "Error", "Unsupported file format")
                return

            self.refresh_preview()
            self.import_btn.setEnabled(True)
        except Exception as e:
            ErrorHandler.show_error(self, "Error", f"Failed to load file: {str(e)}")
...
class PDFGeneratorMixin:
    ...
    def _draw_single_question(self, c, question_index, y, question_height):
        ...
        # Only draw bubbles for non-empty options
    ...
    def update_preview(self):
        try:
            ...
            if hasattr(self, 'preview') and self.preview:
                ...
                print("DEBUG: Preview widget not found")
        except Exception as e:
...
class ScannerWidget(QWidget):
    ...
    def load_image(self):
        ...
        try:
            ...
            # Update display
            print(f"DEBUG: Setting image to display - size: {self.current_image.size}, mode: {self.current_image.mode}")
            ...
            print(f"DEBUG: Image loaded successfully: {filename}")
    ...
    def load_omr(self):
        ...
        try:
            ...
            ErrorHandler.show_error(self, translator.t('error'), f"Failed to load OMR file:\n{str(e)}")
    ...
    def _transform_coordinates(self):
        ...
        if not self.anchors or not self.omr_data:
            print("DEBUG: Missing anchors or OMR data for coordinate transformation")
            return

        bubble_coords = self.omr_data.get('bubble_coordinates', {})
        self.bubble_positions = {}
        print(f"DEBUG: Transforming coordinates for {len(bubble_coords)} questions")

        for q_str, q_data in bubble_coords.items():
            q_num = int(q_str)
            self.bubble_positions[q_num] = {}

            # Check all available options in the bubble coordinates (not just A,B,C,D)
            for option, option_data in q_data.items():
                if isinstance(option_data, dict):  # Make sure it's option data, not metadata
                    rel_info = option_data.get('relative_to_anchor')
        ...
        print(f"DEBUG: Transformed positions for {len(self.bubble_positions)} questions: {list(self.bubble_positions.keys())}")

    def _analyze_bubbles(self):
        """Analyze all bubbles"""
        if not self.current_image or not self.bubble_positions:
            print(f"DEBUG: Cannot analyze - image: {self.current_image is not None}, positions: {len(self.bubble_positions) if self.bubble_positions else 0}")
            return

        print(f"DEBUG: Starting bubble analysis with {len(self.bubble_positions)} questions")
    ...
    def on_analysis_complete(self, result):
        ...
        print(f"DEBUG: Analysis complete called with result: {result}")
        if result['success']:
            self.analysis_results = result['results']
            self.answers = result['answers']
            print(f"DEBUG: Found {len(self.answers)} answers: {self.answers}")
            ...
            print(f"DEBUG: Setting results text to: {text[:100]}...")
            ...
            print(f"DEBUG: Analysis failed with error: {result.get('error', 'Unknown error')}")
    ...
    def toggle_drag_mode(self):
        """Toggle bubble drag mode"""
        try:
            is_enabled = self.drag_mode_btn.isChecked()
            self.image_display.set_drag_mode(is_enabled)
            
            if is_enabled:
                self.drag_mode_btn.setText("🎯 Disable Drag Mode")
                # Set bubble coordinates if available
                if hasattr(self, 'bubble_positions') and self.bubble_positions:
                    # Convert bubble_positions format (x,y) tuples to drag format {x:, y:, radius:} dicts
                    drag_coordinates = {}
                    for q_num, options in self.bubble_positions.items():
                        drag_coordinates[q_num] = {}
                        for option, (x, y) in options.items():
                            drag_coordinates[q_num][option] = {
                                'x': x, 'y': y, 'radius': 20  # Default radius
                            }
                    self.image_display.set_bubble_coordinates(drag_coordinates)
                elif self.omr_data and 'bubble_coordinates' in self.omr_data:
                    self._transform_coordinates()
                    # Convert bubble_positions to drag format
                    drag_coordinates = {}
                    for q_num, options in self.bubble_positions.items():
                        drag_coordinates[q_num] = {}
                        for option, (x, y) in options.items():
            ...
            print(f"ERROR in toggle_drag_mode: {e}")
            import traceback
            traceback.print_exc()
    ...
    def on_bubble_coordinates_updated(self, new_coordinates):
        """Handle bubble coordinates update from drag and drop"""
        try:
            # Convert drag coordinates format {x:, y:, radius:} back to analysis format (x,y) tuples
            converted_positions = {}
            for q_num, question_bubbles in new_coordinates.items():
                converted_positions[q_num] = {}
                for option, bubble_data in question_bubbles.items():
                    if isinstance(bubble_data, dict) and 'x' in bubble_data and 'y' in bubble_data:
                        converted_positions[q_num][option] = (bubble_data['x'], bubble_data['y'])
            
            # Update internal bubble positions
            self.bubble_positions = converted_positions
            
            # Update OMR data if available
            if self.omr_data and 'bubble_coordinates' in self.omr_data:
                # Update the OMR data with new coordinates
                for q_num, question_bubbles in new_coordinates.items():
                    if str(q_num) in self.omr_data['bubble_coordinates']:
                        for option, bubble_data in question_bubbles.items():
                            if option in self.omr_data['bubble_coordinates'][str(q_num)]:
                                self.omr_data['bubble_coordinates'][str(q_num)][option].update(bubble_data)
            
            print(f"DEBUG: Bubble coordinates updated via drag and drop")
        except Exception as e:
            print(f"ERROR in on_bubble_coordinates_updated: {e}")
            import traceback
            traceback.print_exc()
    ...
    def load_scan_results(self):
        ...
        if file_path:
            try:
                ...
                ErrorHandler.show_error(self, translator.t('error'), f"Failed to load results:\n{str(e)}")
...
class OMRUnifiedApp(QMainWindow):
    ...
    def create_status_bar(self):
        ...
        self.theme_label.setStyleSheet(self.THEME_LABEL_STYLE)
    ...
    def toggle_theme(self, event=None):
        ...
        if self.dark_mode:
            ...
            self.theme_label.setStyleSheet(self.THEME_LABEL_DARK_STYLE)
        else:
